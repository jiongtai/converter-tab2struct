...
import (
	"fmt"
	"github.com/jiongtai/converter-tab2struct/db"
	"github.com/jiongtai/converter-tab2struct/tpl"
	"github.com/jiongtai/converter-tab2struct/util"
	"gorm.io/gorm"
	"os"
	"strings"
	"text/template"
...
func doGenerateOne(orm *gorm.DB, conf GenConf, tblName string) (err error) {
	...
	//输出至文件的情况
	fName := fmt.Sprintf("%s/%s.go", conf.WritePath, info.PackageName)
	fmt.Println(fName)
